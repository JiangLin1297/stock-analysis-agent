...
你正在审查这个多Agent股票分析系统的回测结果。你要从"我"的角度检查：
1. 数据是否能跑通
2. 分析逻辑是否太保守
3. 短线机会是否被忽略
4. 长线潜力是否被低估
...
**禁止输出 JSON 代码块。禁止输出 code_changes 数组。**

你必须以"人工执行清单"的形式提供建议。每条建议的格式为：

"请打开 [文件名]，找到 [函数名/变量名]，将其 [参数] 从 [旧值/旧逻辑] 修改为 [新值/新逻辑]，因为 [原因]。"

示例：
- "请打开 analysis/factor_weights.json，找到 short 对象中的 threshold，将其从 55 修改为 48，因为当前阈值过高导致短线信号过少。"
- "请打开 agents/decision.py，找到 generate_factor_signal 函数中的 base_pos 字典，将 short 对应的值从 20 修改为 30，因为短线仓位过保守。"
- "请打开 analysis/holding.py，找到 evaluate_holding 函数中的 ATR_STOP_SHORT 常量，将其从 1.5 修改为 2.5，因为止损太紧导致频繁止损。"

═══════════════════════════════════════════════════════════
【可修改的文件和参数清单】
═══════════════════════════════════════════════════════════

你只能建议修改以下文件中的以下参数（超出范围的建议无效）：

1. analysis/factor_weights.json
   - short/mid/long 对象中的 threshold（信号阈值，当前均为55）
   - short/mid/long 对象中的 expected_return（预期收益目标）

2. agents/decision.py → generate_factor_signal 函数
   - base_pos 字典中的 short/mid/long 值（基础仓位比例）
   - position_pct 计算中的上限值（当前80）
   - 趋势过滤条件（BEAR判断、MA20斜率判断）
   - score_strength 计算公式

3. analysis/holding.py → evaluate_holding 函数
   - ATR_STOP_SHORT / ATR_STOP_MID / ATR_STOP_LONG（止损倍数常量）
   - MA5跌破判断逻辑（当前减半仓还是清仓）
   - MA20趋势转弱判断逻辑
   - RSI超买阈值（当前短线80、中线75）
   - 最大仓位比例 max_position_pct

4. analysis/alpha.py → composite_score 函数
   - 因子权重计算逻辑
   - 基准分（当前50）
